 ...
 * @author eng.mahmoud
 */
class TrainingInspectors extends TCPDF {
...
//    parent::__construct();
    ...
    $this->Cell(540, 13, 'Training Report - Inspectors', 0, 0, 'R');
    ...
    $sql = "SELECT jobs.inspector
                 , jobs.customer
                 , orders_tbl.job
                 , orders_tbl.po
                 , orders_tbl.part
                 , DATE_FORMAT(orders_tbl.began, '%m/%d/%Y') began_date
            FROM jobs
            INNER JOIN orders_tbl
              ON jobs.job = orders_tbl.job
            GROUP BY jobs.inspector, jobs.customer, orders_tbl.job, orders_tbl.po, orders_tbl.part, orders_tbl.began
            HAVING (((jobs.inspector) Is Not Null) AND ((jobs.customer) Like '%' || '{$customer_name}' || '%') AND ((orders_tbl.began)>=Now()-180))
            ORDER BY jobs.inspector;
            ";
//    die($sql);
    $r = $db->query($sql);
    while ($i = $r->fetch_assoc()) {
      $this->Cell(120, 13, $i['began_date'], 0, 0, 'L');
      $this->Cell(120, 13, $i['customer'], 0, 0, 'L');
      $this->Cell(120, 13, $i['job'], 0, 0, 'L');
      $this->Cell(120, 13, $i['po'], 0, 0, 'L');
      $this->Cell(70, 13, $i['part'], 0, 0, 'L');
      $this->Ln();
    }
  }
