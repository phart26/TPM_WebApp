...
class StandardPrices extends TCPDF {
  ...
  private $parts;
  //private $part_speciccrrent;

  public function __construct($customer_id,$part_specic) {
    parent::__construct();
    ...
    //$this->$part_speciccrrent = $part_specic;
...
//    $this->writeHTML($html);
    ...
    $this->parts = [];
    if ($part_specic != ''){
     $sql = "SELECT * FROM part_tbl WHERE cust_id = '$customer_id'  AND part = '$part_specic'";  
    }
    else{
...
//    die($sql);
    ...
    foreach ($this->parts as $part) {
      ...
      $sql = "SELECT cust_tbl.cust_id, cust_tbl.customer, part_tbl.part, part_tbl.dim,
              part_tbl.finished_length, pricing.quantity, part_tbl.description, part_tbl.type, pricing.price
              FROM cust_tbl INNER JOIN part_tbl ON 
              cust_tbl.cust_id = part_tbl.cust_id INNER JOIN pricing ON 
              part_tbl.part = pricing.part WHERE cust_tbl.cust_id = '$customer_id' AND
              part_tbl.part = '$part_no' ORDER BY cust_tbl.customer, part_tbl.part, part_tbl.dim, part_tbl.finished_length, pricing.quantity";
