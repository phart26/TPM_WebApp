...
class MillOperator extends TCPDF {
...
$this->Cell(540, 13, 'Training Report - Mill Operator', 0, 0, 'R');
...
$this->writeHTML('<hr height="2px"/>');
    ...
    $sql = "SELECT 
				...
				orders_tbl.po, 
				orders_tbl.part, 
				orders_tbl.weld_spec_mill, 
				DATE_FORMAT(orders_tbl.began, '%m/%d/%Y') began
			FROM 
				jobs INNER JOIN orders_tbl ON jobs.job = orders_tbl.job
				GROUP BY jobs.mill_operator, jobs.customer, orders_tbl.job, orders_tbl.po, orders_tbl.part, orders_tbl.weld_spec_mill, orders_tbl.began
				HAVING (((jobs.mill_operator) Is Not Null) AND ((jobs.customer) Like '%{$customer_name}%') AND ((orders_tbl.began)>=Now()-180))
				ORDER BY jobs.mill_operator;
            ";
//    die($sql);
